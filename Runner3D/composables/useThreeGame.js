...
import { createAnimatedPlayer, updatePlayerAnimation } from "../game/AnimatedPlayer";
...
export function useThreeGame(canvasRef) {
  ...
  function initThree() {
    ...
    // El jugador se cargará después de forma asíncrona en start()
  ...
  function updatePlayer(dt) {
    // Actualizar animación del modelo 3D
  ...
  function updateWorld(dt) {
    // Aceleración constante y suave (sin saltos bruscos)
    speed.value += dt * 1.0;
    ...
    // Generar obstáculos
    let distanceBetweenObstacles = Math.max(10, 2 * Math.exp(-0.06 * speed.value),);

    // Intervalo basado en velocidad real
    const spawnInterval = distanceBetweenObstacles / speed.value;
  ...
  function start() {
    ...
    createAnimatedPlayer()
      ...
      .catch(error => {
        console.error('Error cargando modelo animado, usando cubo por defecto:', error);
        // Fallback al cubo si falla
        player = createPlayer();
        player.castShadow = true;
        player.position.z = PLAYER_Z;
        scene.add(player);
